...
import { ShieldCheck, Users, TrendingUp, Activity, ExternalLink, Star, SearchX, AlertTriangle, ArrowLeft, Camera, ImageOff, Heart, MessageCircle, Play, ArrowRight } from 'lucide-react'
...
const Stat = ({ icon: Icon, label, value, color }) => (
  <div className="stat-card" style={{ textAlign: 'center', padding: '14px 8px' }}>
    ...
    <p className="tnum" style={{ margin: 0, fontSize: 20, fontWeight: 800, color: 'var(--text)' }}>{value}</p>
    <p style={{ margin: '2px 0 0', fontSize: 10, color: 'rgba(var(--ink-rgb),0.35)' }}>{label}</p>
  </div>
)
  ...
  if (error || !data) return shell(
    <div style={{ textAlign: 'center', padding: 80 }}>
      <div style={{ marginBottom: 12 }}>
        {error === 'notfound'
          ? <SearchX size={40} strokeWidth={1.5} style={{ color: 'rgba(var(--ink-rgb),0.35)' }} />
          : <AlertTriangle size={40} strokeWidth={1.5} style={{ color: 'rgba(var(--ink-rgb),0.35)' }} />}
      </div>
      <h1 style={{ fontSize: 22, fontWeight: 800, color: 'var(--text)' }}>{error === 'notfound' ? 'Creator not found' : 'Something went wrong'}</h1>
      <p style={{ color: 'rgba(var(--ink-rgb),0.5)', marginTop: 8 }}>No FlexTag creator with the handle <strong style={{ color: 'var(--violet-ink)' }}>@{handle}</strong>.</p>
      <Link to="/" className="btn-ghost" style={{ textDecoration: 'none', marginTop: 20, padding: '10px 20px' }}><ArrowLeft size={15} strokeWidth={2} /> Back to FlexTag</Link>
    </div>
  )
  ...
  return shell(
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap', marginBottom: 28 }}>
        <div style={{ width: 84, height: 84, borderRadius: '50%', flexShrink: 0, background: 'var(--purple)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: 32, overflow: 'hidden' }}>
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            {creator.tier && <span style={{ fontSize: 11, fontWeight: 700, color: creator.tier === 'platinum' ? 'var(--violet-ink)' : tierC, background: `${tierC}1f`, border: `1px solid ${tierC}44`, padding: '2px 10px', borderRadius: 16, textTransform: 'capitalize' }}>{creator.tier}</span>}
          </div>
          {creator.instagramHandle && (
            <a href={`https://instagram.com/${creator.instagramHandle}`} target="_blank" rel="noreferrer" style={{ fontSize: 14, color: 'var(--violet-ink)', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 5, marginTop: 4 }}>
      ...
      {posts.length === 0 ? (
        <div className="empty-state"><Camera size={28} strokeWidth={1.5} style={{ color: 'var(--text-dim)', opacity: 0.5, marginBottom: 10 }} /><p>No verified posts yet</p></div>
      ) : (
            ...
            <a key={p._id} href={p.permalink} target="_blank" rel="noreferrer" style={{ borderRadius: 14, overflow: 'hidden', border: '1px solid rgba(var(--ink-rgb),0.06)', background: 'rgba(var(--ink-rgb),0.02)', textDecoration: 'none', display: 'block' }}>
              <div style={{ aspectRatio: '1 / 1', background: 'var(--bg-2)', position: 'relative' }}>
                {p.thumbnail ? <img src={p.thumbnail} alt="" referrerPolicy="no-referrer" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => { e.currentTarget.style.display = 'none' }} />
                  : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.4 }}><ImageOff size={26} strokeWidth={1.5} /></div>}
              ...
              <div style={{ padding: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 10, fontSize: 11, color: 'rgba(var(--ink-rgb),0.4)', marginTop: 4 }}>
                  <span className="tnum" style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}><Heart size={12} strokeWidth={1.75} /> {p.likes == null ? '—' : compact(p.likes)}</span>
                  <span className="tnum" style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}><MessageCircle size={12} strokeWidth={1.75} /> {compact(p.comments)}</span>
                  {p.views != null && <span className="tnum" style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}><Play size={12} strokeWidth={1.75} /> {compact(p.views)}</span>}
                </div>
              </div>
            </a>
          ))}
      ...
      <div style={{ marginTop: 40, textAlign: 'center', padding: '28px 20px', borderRadius: 16, background: 'rgba(124,58,237,0.06)', border: '1px solid rgba(124,58,237,0.2)' }}>
        ...
        <Link to="/register?role=creator" className="btn-primary" style={{ textDecoration: 'none', padding: '11px 24px' }}>Start earning on FlexTag <ArrowRight size={15} strokeWidth={2} /></Link>
      </div>
    </>
  )
}
