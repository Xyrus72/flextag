...
const BrandDashboard = () => {
  ...
  const kpis = [
    { label:'Active Campaigns',   value:String(stats.activeCampaigns),                         Icon: Megaphone, color:'rgba(124,58,237,0.12)', border:'rgba(124,58,237,0.25)', text:'#a78bfa', sub:'Live now' },
    { label:'Total Creators',     value:String(stats.totalCreators),                            Icon: Users, color:'rgba(6,182,212,0.12)',  border:'rgba(6,182,212,0.25)',  text:'#67e8f9', sub:'Joined campaigns' },
    { label:'Cashback Disbursed', value:`৳${(stats.cashbackDisbursed||0).toLocaleString()}`,    Icon: Banknote, color:'rgba(34,197,94,0.12)', border:'rgba(34,197,94,0.25)',  text:'#4ade80', sub:'Total paid out' },
    { label:'Avg. ROI',           value:'—',                                                    Icon: BarChart3, color:'rgba(236,72,153,0.12)',border:'rgba(236,72,153,0.25)',  text:'#f9a8d4', sub:'No data yet' },
  ]
              ...
              {recentOrders.map(o => (
                <div key={o._id} style={{ display:'flex', alignItems:'center', gap:14, padding:'14px 16px', borderRadius:14, background:'rgba(var(--ink-rgb),0.02)', border:'1px solid rgba(var(--ink-rgb),0.05)' }}>
                  ...
                  <div style={{ textAlign:'right', flexShrink:0 }}>
                    <p style={{ fontSize:14, fontWeight:700, color: 'var(--text)', margin:0 }}>৳{o.total?.toLocaleString()}</p>
