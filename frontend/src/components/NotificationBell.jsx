    ...
    <div ref={ref} style={{ position: 'relative' }}>
      <button type="button" onClick={toggle} aria-label="Notifications" style={{
        ...
        {unread > 0 && (
          <span style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #050816',
      ...
      {open && (
        <div style={{
          ...
          background: '#0d1024', border: '1px solid rgba(124,58,237,0.25)', borderRadius: 16,
          boxShadow: '0 20px 50px rgba(0,0,0,0.5)', zIndex: 60, padding: 8,
        }}>
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', padding: '8px 10px 6px' }}>Notifications</p>
          {items.length === 0 ? (
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', textAlign: 'center', padding: '24px 12px' }}>You&apos;re all caught up 🎉</p>
          ) : items.map((n, i) => (
            <button type="button" key={n._id || i} onClick={() => openItem(n)} style={{
              ...
              <span style={{ minWidth: 0, flex: 1 }}>
                <span style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#fff' }}>{n.title}</span>
                {n.body && <span style={{ display: 'block', fontSize: 12, color: 'rgba(255,255,255,0.5)', lineHeight: 1.4, marginTop: 2 }}>{n.body}</span>}
              </span>
              <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', flexShrink: 0 }}>{relative(n.createdAt)}</span>
            </button>
          ))}
        </div>
      )}
