  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <div>
          <p style={{ margin: 0, fontSize: 14, fontWeight: 800, color: 'var(--text)' }}>🎁 Refer a creator, both earn ৳{data.bonus}</p>
          <p style={{ margin: '4px 0 0', fontSize: 12, color: 'rgba(var(--ink-rgb),0.55)' }}>
            Paid when they complete their first campaign · {data.count} referred{data.rewarded ? ` · ${data.rewarded} earned` : ''}
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <code style={{ fontSize: 13, fontWeight: 800, letterSpacing: '0.08em', color: 'var(--text)', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(var(--ink-rgb),0.12)', borderRadius: 10, padding: '8px 12px' }}>{data.code}</code>
